...
const ALL_PATTERN_TOKENS = ['segment', 'gap', 'timed', 'mobile', 'hazard', 'launch'];
const TIMED_KINDS = new Set(['beat', 'alternateBeat', 'ghost', 'reverseGhost']);
const LAUNCH_KINDS = new Set(['launch30', 'launch60']);
const PATTERN_REPEAT_COOLDOWN_PICKS = 3;
const PATTERN_REPEAT_MIN_WEIGHT_FACTOR = 0.3;
const DIVERSITY_KIND_PRIORITY = [
    'beat',
    'alternateBeat',
    'ghost',
    'reverseGhost',
...
function weightedPick(items, getWeight, roll) {
    if (items.length <= 1)
        return items[0];
...
function normalizePatternToken(token) {
    if (typeof token !== 'string')
        return null;
    return ALL_PATTERN_TOKENS.includes(token) ? token : null;
}
function toPatternTokens(rawTokens) {
    ...
    if (Array.isArray(rawTokens[0])) {
        ...
        for (let i = rows.length - 1; i >= 0; i--) {
            ...
            if (!Array.isArray(row) || row.length <= 0)
                continue;
            const tokens = row.map(normalizePatternToken).filter((token) => token !== null);
            if (tokens.length > 0)
                return tokens;
        }
        return [];
    }
    return rawTokens.map(normalizePatternToken).filter((token) => token !== null);
}
function sanitizeEnergyHint(value) {
    if (value === 'low' || value === 'medium' || value === 'high')
        return value;
    return 'medium';
}
function buildDefinedPatternCatalog() {
    const source = (Array.isArray(RUNTIME_PATTERN_CATALOG) ? RUNTIME_PATTERN_CATALOG : []);
    const patterns = [];
    for (let i = 0; i < source.length; i++) {
        const row = source[i];
        const kind = row?.kind;
        if (kind !== 'flow1d' && kind !== 'micro2d')
            continue;
...
function patternPoolForEnergy(energy) {
...
function repeatPenaltyForPattern(patternId, recentPatternIds) {
...
function toUniqueRuntimePlatformTypes(platformTypes) {
...
function applyEnergyGate(seg, enabled) {
    ...
    const filtered = toUniqueRuntimePlatformTypes(seg.platformTypes).filter((platformType) => {
        if (platformType === 'static')
            return true;
        return allowed.has(platformType);
    });
    return {
        ...seg,
...
function selectPatternTokens(segments, seed) {
    ...
    while (index < segments.length) {
        ...
        const pickedPattern = weightedPick(pool, (pattern) => pattern.weight *
            (0.75 + clampUnit(seg.rhythmDensity) * 0.5) *
            repeatPenaltyForPattern(pattern.patternId, recentPatternIds), deterministicUnit(seed + (index + 1) * 31.13));
        recentPatternIds.push(pickedPattern.patternId);
        if (recentPatternIds.length > PATTERN_REPEAT_COOLDOWN_PICKS)
            recentPatternIds.shift();
        patternPicks.push({
            segmentIndex: index,
    ...
    const patternUsage = [...usageMap.entries()]
        ...
        if (b.segmentsCovered !== a.segmentsCovered)
            return b.segmentsCovered - a.segmentsCovered;
        return a.patternId.localeCompare(b.patternId);
    });
    return { patternPoolSizes, patternPicks, patternUsage, tokens };
}
function chooseKindFromToken(token, seg, seed) {
    if (token === 'segment')
        return 'segment';
    if (token === 'gap')
        return null;
    if (token === 'hazard') {
        if (supportsHazard(seg))
            return 'hazard';
        if (supportsTimed(seg))
            return chooseKindFromToken('timed', seg, seed + 7.1);
        if (supportsMobile(seg))
            return chooseKindFromToken('mobile', seg, seed + 11.7);
        return 'segment';
    }
    if (token === 'launch') {
        const candidates = [];
        if (hasRuntimeKind(seg, 'launch30'))
            candidates.push('launch30');
        if (hasRuntimeKind(seg, 'launch60'))
            candidates.push('launch60');
    ...
    if (token === 'mobile') {
        const candidates = [];
        if (hasRuntimeKind(seg, 'elevator'))
            candidates.push('elevator');
        if (hasRuntimeKind(seg, 'shuttle'))
            candidates.push('shuttle');
        if (hasRuntimeKind(seg, 'cross'))
            candidates.push('cross');
        if (hasRuntimeKind(seg, 'spring'))
    ...
    if (hasRuntimeKind(seg, 'beat'))
        timedCandidates.push('beat');
    if (hasRuntimeKind(seg, 'alternateBeat'))
        timedCandidates.push('alternateBeat');
    if (hasRuntimeKind(seg, 'ghost'))
        timedCandidates.push('ghost');
    if (hasRuntimeKind(seg, 'reverseGhost'))
        timedCandidates.push('reverseGhost');
    if (timedCandidates.length <= 0)
        return 'segment';
    const density = clampUnit(seg.rhythmDensity);
    return weightedPick(timedCandidates, (kind) => {
        if (kind === 'beat' || kind === 'alternateBeat')
            return 0.8 + density * 1.1;
        return 0.65 + density * 0.95;
    }, deterministicUnit(seed + 71.2));
}
function pickAnyNonStaticKind(seg, seed) {
    const candidates = [];
    for (const kind of DIVERSITY_KIND_PRIORITY) {
        if (!hasRuntimeKind(seg, kind))
            continue;
        if (kind === 'hazard' && seg.energyState === 'low')
            continue;
        if ((kind === 'launch30' || kind === 'launch60') && seg.energyState !== 'high')
            continue;
...
function satisfiesResolvedConstraints(kinds, config) {
    if (kinds.length > 0 && (kinds[0] === null || kinds[kinds.length - 1] === null))
        return false;
    ...
    for (let i = 0; i < kinds.length; i++) {
        ...
        if (i > 0) {
            for (const [fromKind, toKind] of config.forbidPairs) {
                if (kinds[i - 1] === fromKind && kind === toKind)
                    return false;
            }
        }
...
function enforceHighVarietyKinds(kinds, segments, config, seed) {
    const out = [...kinds];
    ...
    if (targetDiversity <= 0)
        return out;
    const usedKinds = new Set(out.filter((kind) => kind !== null && kind !== 'segment'));
    if (usedKinds.size >= targetDiversity)
        return out;
    for (const missingKind of DIVERSITY_KIND_PRIORITY) {
        if (!availableKinds.has(missingKind) || usedKinds.has(missingKind))
            continue;
        const slots = [];
        for (let i = 1; i < out.length - 1; i++) {
            ...
            if (!seg || !hasRuntimeKind(seg, missingKind))
                continue;
            if (missingKind === 'hazard' && seg.energyState === 'low')
                continue;
            if ((missingKind === 'launch30' || missingKind === 'launch60') && seg.energyState !== 'high')
                continue;
            slots.push(i);
        }
        if (slots.length <= 0)
        ...
        for (const slot of slots) {
            ...
            if (!satisfiesResolvedConstraints(trial, config))
                continue;
...
function enforceMinimumLaunchKinds(kinds, segments, config, seed) {
    ...
    for (let i = 1; i < out.length - 1; i++) {
        if (out[i] !== 'segment')
            continue;
        const seg = segments[i];
        if (!seg || seg.energyState !== 'high' || !supportsLaunch(seg))
            continue;
        launchSlots.push(i);
    }
    ...
    for (const slot of launchSlots) {
        ...
        const launchKinds = [];
        if (hasRuntimeKind(seg, 'launch30'))
            launchKinds.push('launch30');
        if (hasRuntimeKind(seg, 'launch60'))
            launchKinds.push('launch60');
        if (launchKinds.length <= 0)
            continue;
...
function enforceMinimumHazardKind(kinds, segments, config, seed) {
    const out = [...kinds];
    ...
    for (let i = 1; i < out.length - 1; i++) {
        if (out[i] !== 'segment')
            continue;
        const seg = segments[i];
        if (!seg || seg.energyState !== 'high' || !supportsHazard(seg))
            continue;
        slots.push(i);
    }
    if (slots.length <= 0)
...
function applyKindConstraints(kinds, segments, config, seed) {
    const out = [...kinds];
    if (out.length > 0) {
        if (out[0] === null)
            out[0] = 'segment';
        if (out[out.length - 1] === null)
    ...
    for (let pass = 0; pass < 3; pass++) {
        ...
        for (let i = 0; i < out.length; i++) {
            ...
            if (postKind !== null && LAUNCH_KINDS.has(postKind)) {
                ...
                if (Number.isFinite(segmentsSinceLaunch))
                    segmentsSinceLaunch += 1;
            }
        }
    }
    ...
    if (satisfiesResolvedConstraints(hazardEnforced, config))
        return hazardEnforced;
    // Final conservative cleanup: convert conflicting entries to static segments.
    const safe = [...hazardEnforced];
    if (safe.length > 0) {
        if (safe[0] === null)
            safe[0] = 'segment';
        if (safe[safe.length - 1] === null)
            safe[safe.length - 1] = 'segment';
    }
    let segmentsSinceLaunch = Number.POSITIVE_INFINITY;
    let timedRun = 0;
...
export function generatePlatformKindSequenceWithTrace(segments, seed = 1, rules = defaultKindSequenceRules) {
    const resolved = resolveKindRules(rules);
    const gated = segments.map((seg) => applyEnergyGate({
        ...seg,
