    ...
    create() {
        ...
        for (const platform of this.currentLevel.platforms) {
            if (platform.kind === 'segment') {
                ...
                const beat = this.add.rectangle(this.snapXToGrid(platform.x), this.snapYToGrid(platform.y), this.snapLengthToGrid(platform.width), this.platformBlockHeight, 0x8b93aa, 0.95);
                this.beatPlatforms.push(beat);
            }
            else if (platform.kind === 'alternateBeat') {
                const alternateBeat = this.add.rectangle(this.snapXToGrid(platform.x), this.snapYToGrid(platform.y), this.snapLengthToGrid(platform.width), this.platformBlockHeight, 0xffb86c, 0.95);
                this.alternateBeatPlatforms.push(alternateBeat);
            }
            else if (platform.kind === 'ghost') {
                const ghost = this.add.rectangle(this.snapXToGrid(platform.x), this.snapYToGrid(platform.y), this.snapLengthToGrid(platform.width), this.platformBlockHeight, 0x73f7ff, 0.15);
                this.ghostPlatforms.push(ghost);
            }
            else if (platform.kind === 'reverseGhost') {
                const reverseGhost = this.add.rectangle(this.snapXToGrid(platform.x), this.snapYToGrid(platform.y), this.snapLengthToGrid(platform.width), this.platformBlockHeight, 0xff8cf7, 0.85);
                this.reverseGhostPlatforms.push(reverseGhost);
            }
            else if (platform.kind === 'elevator') {
                const elevator = this.add.rectangle(this.snapXToGrid(platform.x), this.snapYToGrid(platform.y), this.snapLengthToGrid(platform.width), this.platformBlockHeight, 0x60a5fa, 0.95);
